...
class MY_Model extends CI_Model {
	 ...
	 * @return boolean
	 */
	public function update_data(Array $pairs,  $where,$table) {
		return $this->db->update ( $table, $pairs, $where );
	}
        
        /**
         * 更新数据，条件为where_in
         * @param string $fields where in 字段
         * @param array $where_in where in 条件
         * @param array $pairs 更新数据
         * @param array $where 更新条件
         * @param string $table 表名
         * @return type
         */
        public function update_where_in($fields,$where_in,Array $pairs, Array $where,$table)
        {
            $this->db->where_in($fields, $where_in);
            return $this->db->update ( $table, $pairs, $where );
        }
	 ...
	 * @param array|string $where
	 */
	public function count($key, $where,$table){
		$this->db->select ( "count({$key}) as total" );
		$this->db->where ( $where );
		$query = $this->db->get ( $table );
		$row = $query->row_array ();
	...
	public function get($name){
         ...
         * LEFT JOIN
         */
        public function get_composite_row_array($condition, $join_condition,$select,$tb_a,$tb_b,$batch = FALSE,$limit = 0)
        {
